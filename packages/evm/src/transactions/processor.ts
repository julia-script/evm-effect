/**
 * Transaction processing functions for the Osaka fork.
 *
 * Ports the transaction processing logic from the Python reference implementation
 * in fork.py, specifically the process_transaction function.
 */
...
export const processTransaction: (
  ...
  | Type3TxPreForkError,
  Fork
> = Effect.fn("processTransaction")(
  function* (blockEnv, blockOutput, tx, index) {
    yield* annotateSafe({
      blockEnv: blockEnv,
      // "blockOutput": blockOutput,
      tx: tx,
    ...
    // Step 1: Add transaction to transactions trie with RLP encoding
    ...
    const { calldataFloorGasCost, intrinsicGas } =
      yield* validateTransaction(tx);
    yield* annotateSafe({
    ...
    // EIP-3529 (London fork) changed refund cap from 1/2 to 1/5
    ...
    // EIP-161: Destroy touched empty accounts (state trie clearing)
    // Only applies from Spurious Dragon onwards
    if (fork.eip(161)) {
    ...
    // Create receipt - format depends on fork (EIP-658 changed receipt format in Byzantium)
    let encodedReceiptResult: Either.Either<Bytes, unknown>;

    if (fork.eip(658)) {
      // Byzantium+: Use status code (succeeded: boolean)
      ...
      // Pre-Byzantium: Use post-transaction state root
    ...
    // Add type prefix for typed transactions
    let encodedReceipt: Bytes;
    switch (tx._tag) {
      case "LegacyTransaction":
        // Legacy transactions: receipt is just the RLP encoding
