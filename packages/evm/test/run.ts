...
const loaded = (await readFile(fixturesIndexPath, {
  encoding: "utf-8",
}).then(JSON.parse)) as (typeof Index)["Type"];
...
import { emptyBlockOutput } from "../src/blockchain.js";
...
if (filter) {
  stateTests = stateTests.filter(
    (testCase) =>
      testCase.shortHash?.includes(filter) || testCase.id.includes(filter),
  );
}
...
for (const testCase of stateTests) {
  ...
  //   `$shortHash - [$index] $id`,
  // async (testCaseIndex) => {
  testState.shortHash = testCase.shortHash;
  testState.passed = false;
  testState.skipped = false;
  ...
  const testCaseRaw: unknown = (
    await readFile(path.join(root, "fixtures", testCase.json_path), {
      encoding: "utf-8",
    }).then(JSON.parse)
  )[testCase.id];
    ...
    if (testFailed) {
      ...
      for (const [address, expectedAccount] of Object.entries(post.state)) {
        ...
        if (actualAccount.balance.value !== expectedAccount.balance.value) {
          ...
          yield* Console.log(`      Actual:   ${actualAccount.balance.value}`);
          const offset =
  ...
  try {
    await Effect.gen(function* () {
      yield* Effect.log(`Running test case: ${testCase.id}`);
